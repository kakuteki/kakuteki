...
Uses hardcoded values until Kaggle API access is configured.
To update: edit the profile_data values below.
"""
...
def generate_svg(cat, tier, rank, medals):
    ...
    medal_parts = []
    if g:
        medal_parts.append(f'G{g}')
    if s:
        medal_parts.append(f'S{s}')
    if b:
        medal_parts.append(f'B{b}')
    m = ' '.join(medal_parts)
...
def main():
    # Update these values manually when your Kaggle stats change
    profile_data = {
        'username': 'kakuteki',
        'competitions': {
            'tier': 'Novice',
